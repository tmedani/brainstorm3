                ...
                if isempty(strfind(filenameRelative, 'headmodel_grid_'))
                    ...
                    for mod = {'MEG', 'EEG', 'SEEG', 'ECOG'}
                        ...
                        if strcmpi(sStudy.HeadModel(iHeadModel).HeadModelType, 'volume')
                            ...
                            gui_component('MenuItem', jPopup, [], ['View ' mod{1} ' leadfield vectors as FEM tensors, ellip'], IconLoader.ICON_FEM, [], @(h,ev)bst_call(@view_tensors, filenameRelative, 'ellipse', mod{1}));
                            AddSeparator(jPopup);
